...
class HealthCheck {
   ...
   * @param {AsyncFunction} checkerAsyncFun - an async function to call for getting values
   * @param {int} minCacheMs - min cache duration in milliseconds.
   */
  addCheck (componentName, metricName, checkerAsyncFun, minCacheMs=0) {
    const key = `${componentName}:${metricName}`;
    const newCheck = {};
    newCheck.key = key;
    newCheck.executor = checkerAsyncFun;
  ...
  getChecksForCurrentRun() {
    ...
    resp.promises = currRun.map(check => {
      return check.executor(); // we need to return an unresolved promise from async fun
    });
